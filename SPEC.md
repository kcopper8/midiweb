...
#### 10.1 기본 동작 {#ACCOMP-01}
- 반주 토글 버튼으로 on/off
- 메트로놈 시작 시 반주도 함께 시작 (반주 on 상태일 때)
- 메트로놈 정지 시 반주도 함께 정지
- 매 마디의 첫 박에 해당 코드의 구성음을 동시에 재생

#### 10.2 코드 재생 {#ACCOMP-02}
- 코드 구성음 3개를 동시에 재생 (3화음)
- 볼륨: 0.15 (메트로놈/연주보다 낮게)
- 파형: sine (부드러운 소리)
- 옥타브: C3 기준 (연주보다 낮게)

#### 10.3 코드 진행 프리셋 {#ACCOMP-03}
...
| blues | Blues (12bar) | I-I-I-I-IV-IV-I-I-V-IV-I-V | 12마디 블루스 |
...
#### 11.1 기본 동작 {#DRUM-01}
- 반주와 통합: 반주 on/off와 함께 동작
- 메트로놈 시작 시 드럼도 함께 시작 (반주 on 상태일 때)
- 16분음표 단위로 패턴 재생

#### 11.2 드럼 사운드 합성 {#DRUM-02}
Web Audio API로 합성:
- **킥 (Kick)**: 저주파 사인파(150Hz→50Hz) + 클릭 어택
- **스네어 (Snare)**: 삼각파(200Hz) + 고주파 화이트 노이즈
- **하이햇 (Hi-hat)**: 밴드패스 필터된 노이즈 (8kHz)

#### 11.3 드럼 패턴 프리셋 {#DRUM-03}
코드 진행 프리셋과 1:1 매칭 (16분음표 기준):

| ID | 이름 | 킥 | 스네어 | 하이햇 | 특징 |
|----|------|-----|--------|--------|------|
| pop | Pop | 1, 9 | 5, 13 | 짝수 | 기본 8비트 |
| classic | Classic | 1 | 9 | 1, 5, 9, 13 | 단순 4비트 |
| sad | Sad | 1, 11 | 5, 13 | 모든 스텝 | 느린 발라드 |
| jazz | Jazz | 1, 7 | 5, 13 | 스윙 패턴 | 셔플 느낌 |
| fifties | 50s | 매 박 | 5, 13 | 짝수 | 4온더플로어 |
| blues | Blues | 1, 10 | 5, 13 | 셔플 | 스윙 블루스 |

#### 11.4 UI {#DRUM-04}
- 반주 프리셋 드롭다운 우측에 드럼 볼륨 슬라이더
- 🥁 아이콘 + 슬라이더 (0~100%)
- 볼륨 0%로 드럼만 음소거 가능
